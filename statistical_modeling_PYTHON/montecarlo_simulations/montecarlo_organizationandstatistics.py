...
dirname = genotype +'_Output_'+ time.strftime("%Y%m%d_%H%M%S")
os.mkdir(dirname)
os.chdir(dirname)
...
for iteration in range(0, int(cells_to_simulate)):
    ...
    for cellnumber, __ in enumerate(positionframe.T):
        ...
        for alphano, elem in enumerate(idx_alphas):
            ...
            positions, distances = GenerateDistributions_Uniformrandom(positionframe.iloc[cellnumber][elem], sigmaframe.ix[positionframe.iloc[cellnumber].name][elem], celllength)
